...
function str2buf(str, enc) {
  ...
  return enc
    ? Buffer.from(str, enc)
    : Buffer.from(base_58_1.default.decode(str));
}
/**
...
function encrypt(plaintext, key, iv, algo) {
  ...
  if (Buffer.isBuffer(plaintext)) {
    ...
    var text = str2buf(plaintext);
  } else {
    throw new Error(plaintext + " must be either a string or a buffer");
  }
  const cipher = crypto_1.default.createCipheriv(algo, keyBuff, iv);
...
function create(params, cb) {
  ...
  function curve25519KeyGen(randomBytes) {
    ...
    return {
      ...
      iv: bifrostBlake2b(
        crypto_1.default.randomBytes(keyBytes + ivBytes + keyBytes)
      ).slice(0, ivBytes),
  ...
  if (cb) {
    if (isFunction(cb)) {
      crypto_1.default.randomBytes(keyBytes + ivBytes + keyBytes, function (
        randomBytes
      ) {
...
function deriveKey(password, salt, kdfParams, cb) {
  ...
  if (cb) {
    if (!isFunction(cb)) {
      return crypto_1.default.scryptSync(
...
class KeyManager {
  ...
  constructor(params) {
    ...
    const initKeyStorage = (keyStorage, password) => {
      ...
      if (this.pk) {
        __classPrivateFieldSet(
          this,
          _sk,
          recover(String(password), keyStorage, this.constants.scrypt)
        );
      }
    };
    ...
    const importFromFile = (filepath, password) => {
      const keyStorage = JSON.parse(
        String(fs_1.default.readFileSync(filepath))
      );
  ...
  sign(message) {
    ...
    function curve25519sign(privateKey, message) {
      return curve25519.sign(
        ...
        crypto_1.default.randomBytes(64)
      );
    }
